...
`oracle_residual_inference` after task-153 moved the largest remaining
chain-contour residual from chain48 to chain34. Final regression keeps
...
`0`/`none`; benchmark-05 remains a true `mismatch` with `Lpp` delta `0.443206`,
six summary mismatches, and the largest remaining chain-contour residual now
on chain36 (`max chain contour delta = 2.44735`). Start the next reducer slice
from chain36 final geometry and contour placement. Before each reducer change,
