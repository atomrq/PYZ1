...
- default geometrical Z1+ numerical parity across all benchmarks
  ...
  task-116 aligns chain3 to the pair25 contact while exposing the next chain25
  reciprocal/source-placement gap; these cumulatively improve benchmark-05
  node-count, pair-count, `Lpp`, prior `Z`, and local source residuals while
  leaving task-116's local `Z` regression, geometry, remaining reciprocal
