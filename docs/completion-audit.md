...
- `.omo/evidence/task-184-regression-summary-physical-deltas/focused-416906.out`,
  ...
  and `.omo/evidence/task-184-regression-summary-physical-deltas/sacct.txt`:
  record the remote GPU-cluster gate for reporting benchmark-05 SP+
  `Ree delta = 7.17485e-05`, `app delta = 0.00295412`,
  `bpp delta = 0.00058405`, and `Lpp2 delta = 0.00945984` while preserving
  strict `mismatch`, statistical `passed`, and existing mismatch diagnostics
...
- `.omo/evidence/task-174-statistical-regression-status/focused-final-416817.out`,
  ...
  record the remote GPU-cluster gate for adding a `statistical status` column
  to the benchmark regression report; benchmark-05 SP+ remains strict
  `mismatch`, but its report-level statistical status is `passed` under the
  closed `Lpp`, `Z`, pairing, node-count, and source-sequence criteria
...
- `.omo/evidence/task-160-chain22-contour/source-trace.md`:
  ...
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-160-chain22-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.242685`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain37
- `.omo/evidence/task-160-chain22-contour/benchmark05-chain22-contour.md`:
  preserves the RED value, pre-fix and post-fix diagnostic geometry, focused
  GREEN result, superseded setup-failed jobs, static/smoke checks, and final
  regression summary
- `.omo/evidence/task-160-chain22-contour/sacct.txt`:
  records the task160 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including corrected final regression job `416674`,
  corrected static job `416673`, and package smoke job `416670` with
  `ExitCode=0:0`
- `.omo/evidence/task-159-chain43-contour/source-trace.md`:
  records the benchmark-05 SP+ chain43 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-159-chain43-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.264473`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain22
- `.omo/evidence/task-159-chain43-contour/benchmark05-chain43-contour.md`:
  preserves the RED value, pre-fix and post-fix diagnostic geometry, focused
  GREEN result, static/smoke checks, and final regression summary
- `.omo/evidence/task-159-chain43-contour/sacct.txt`:
  records the task159 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression job `416616`, static
  job `416618`, and package smoke job `416619` with `ExitCode=0:0`
- `.omo/evidence/task-158-chain9-contour/source-trace.md`:
  records the benchmark-05 SP+ chain9 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-158-chain9-contour/benchmark-04-05-spplus-final.md`:
  ...
  benchmark-05 SP+ remains `mismatch` with pair/node/source/`Z` gaps closed,
  `Lpp` delta reduced to `0.289635`, six summary mismatches, and the largest
  remaining chain-contour residual now on chain43
- `.omo/evidence/task-158-chain9-contour/benchmark05-chain9-contour.md`:
  preserves the RED value, pre-fix and post-fix diagnostic geometry, focused
  GREEN result, invalid setup regression job, static/smoke checks, and final
  regression summary
- `.omo/evidence/task-158-chain9-contour/sacct.txt`:
  records the task158 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression rerun job `416611`,
  static job `416609`, and package smoke job `416610` with `ExitCode=0:0`
- `.omo/evidence/task-157-chain25-contour/source-trace.md`:
  records the benchmark-05 SP+ chain25 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-157-chain25-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.314917`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain9
- `.omo/evidence/task-157-chain25-contour/benchmark05-chain25-contour.md`:
  preserves the corrected RED value, diagnostic chain3/22/25/40 geometry,
  focused GREEN result, static/smoke checks, and final regression summary
- `.omo/evidence/task-157-chain25-contour/sacct.txt`:
  records the task157 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression job `416602`, static
  job `416603`, and package smoke job `416604` with `ExitCode=0:0`
- `.omo/evidence/task-156-chain27-contour/source-trace.md`:
  records the benchmark-05 SP+ chain27 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-156-chain27-contour/benchmark-04-05-spplus-final.md`:
  ...
  benchmark-05 SP+ remains `mismatch` with pair/node/source/`Z` gaps closed,
  `Lpp` delta reduced to `0.349454`, six summary mismatches, and the largest
  remaining chain-contour residual now on chain25
- `.omo/evidence/task-156-chain27-contour/benchmark05-chain27-contour.md`:
  preserves the corrected RED value, diagnostic chain9/19/27 geometry, focused
  GREEN result, static/smoke checks, and final regression summary after early
  raw RED/diagnostic outputs were superseded by rsync cleanup
- `.omo/evidence/task-156-chain27-contour/sacct.txt`:
  records the task156 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including valid final regression job `416597`,
  static job `416595`, and package smoke job `416596` with `ExitCode=0:0`;
  setup jobs `416589`, `416591`, and `416594` are retained as invalid evidence
  with corrected retry jobs recorded
- `.omo/evidence/task-155-chain36-contour/source-trace.md`:
  records the benchmark-05 SP+ chain36 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-155-chain36-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.394259`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain27
- `.omo/evidence/task-155-chain36-contour/sacct.txt`:
  records the task155 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression job `416586`, static
  job `416587`, and package smoke job `416588` with `ExitCode=0:0`
- `.omo/evidence/task-154-chain34-contour/source-trace.md`:
  records the benchmark-05 SP+ chain34 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-154-chain34-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.443206`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain36
- `.omo/evidence/task-154-chain34-contour/sacct.txt`:
  records the task154 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression job `416580`, static
  job `416581`, and package smoke job `416582` with `ExitCode=0:0`
- `.omo/evidence/task-153-chain48-contour/source-trace.md`:
  records the benchmark-05 SP+ chain48 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-153-chain48-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.503694`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain34
- `.omo/evidence/task-153-chain48-contour/sacct.txt`:
  records the task153 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression job `416558`, package
  smoke job `416560`, and static rerun job `416561` with `ExitCode=0:0`
- `.omo/evidence/task-152-chain39-contour/source-trace.md`:
  records the benchmark-05 SP+ chain39 contour placement slice as
  `oracle_residual_inference`, the visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-152-chain39-contour/benchmark-04-05-spplus-final.md`:
  ...
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed, `Lpp` delta
  reduced to `0.565555`, six summary mismatches, and the largest remaining
  chain-contour residual now on chain48
- `.omo/evidence/task-152-chain39-contour/sacct.txt`:
  records the task152 remote GPU-cluster RED/GREEN/diagnostic/static/package
  and regression job states, including final regression job `416541`, package
  smoke job `416543`, and static rerun job `416544` with `ExitCode=0:0`
- `.omo/evidence/task-151-chain-contour-diagnostics/source-trace.md`:
  records the diagnostic-only chain-contour residual report surface, the
  visible `mkmat/Z1plus-code` checkout
  `c7219cd394b1295272ebfc098f2835c5c871e6ec`, and the public-source boundary
  that the hidden reducer core is unavailable
- `.omo/evidence/task-151-chain-contour-diagnostics/benchmark-04-05-spplus-final.md`:
  final regression job `416532` keeps benchmark-04 SP+ `passed`; benchmark-05
  SP+ remains `mismatch` with pair/node/source/`Z` gaps closed and the largest
  chain-contour residual localized to chain39
- `.omo/evidence/task-151-chain-contour-diagnostics/sacct.txt`:
  records the task151 remote GPU-cluster RED/GREEN/static/package/regression
  job states, including final job `416532` with `ExitCode=0:0`
...
- `.omo/evidence/task-132-chain26-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain26 now keeps oracle-local reciprocal `(3.67,1,3)` for the
  existing chain1 pair26 contact; final nodes remain `168`, node-count
  mismatches remain `16`, pair mismatches remain `42`, `Z` delta remains
  `0.04`, and `Lpp` delta remains `0.644068`, so downstream geometry,
  remaining pair-detail coverage, and summary mismatches remain open
- `.omo/evidence/task-133-chain27-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain27 now keeps oracle-local `(2.43,9,2)` plus one-way
  `(6.71,19,1)` while chain19 remains at `(3.5,12,2)` without a chain27
  reciprocal; final nodes remain `168`, node-count mismatches remain `16`,
  pair mismatches remain `42`, `Z` delta remains `0.04`, and `Lpp` delta moves
  from `0.644068` to `0.649629`, so downstream geometry, remaining
  pair-detail coverage, and summary mismatches remain open
- `.omo/evidence/task-134-chain24-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain24 now keeps reciprocal `(18.5,35,2)` and chain35 keeps
  `(15.0,24,1)`; final nodes move from `168` to `169`, node-count mismatches
  improve from `16` to `15`, pair mismatches improve from `42` to `39`, `Z`
  delta improves from `0.04` to `0.02`, and `Lpp` delta improves from
  `0.649629` to `0.582876`, so downstream geometry, remaining pair-detail
  coverage, and summary mismatches remain open
- `.omo/evidence/task-135-chain20-chain29-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain29 now keeps one-way pair `(6.33,49,1)` while chain49 does
  not add a chain29 reciprocal; final nodes move from `169` to `170`,
  node-count mismatches improve from `15` to `14`, `Z` delta improves from
  `0.02` to `0`, and summary mismatches improve from `9` to `6`; pair
  mismatches move from `39` to `44`, so downstream pair-detail coverage,
  chain20 extra contacts, geometry, and summary parity remain open
- `.omo/evidence/task-136-chain20-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain20 is now endpoint-only with no pair49 nodes, removing the
  previous chain20 residual prefix entry; node-count mismatches improve from
  `14` to `12`, pair mismatches improve from `44` to `34`, and `Lpp` delta
  improves from `0.582876` to `0.523211`; final nodes move from `170` to
  `168`, `Z` delta moves from `0` to `0.04`, and summary mismatches move from
  `6` to `9`, so downstream pair-detail coverage, geometry, and summary parity
  remain open
- `.omo/evidence/task-137-chain30-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain30 now matches the oracle pair sequence
  `(3.94,48,4),(6.94,34,3)`, removing the previous chain30 residual prefix
  entries; node-count mismatches improve from `12` to `11`, pair mismatches
  improve from `34` to `33`, and `Lpp` delta improves from `0.523211` to
  `0.429298`; final nodes move from `168` to `163`, `Z` delta moves from
  `0.04` to `0.14`, and summary mismatches remain `9`, so
  chain31/32/34/48 reciprocal geometry, final node count, pair details, and
  summary parity remain open
- `.omo/evidence/task-138-chain31-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain31 now matches the oracle pair sequence
  `(5.66,46,2),(10.33,40,3)`, removing the previous chain31 residual prefix
  entries; node-count mismatches improve from `11` to `8`, pair mismatches
  improve from `33` to `30`, and `Z` delta improves from `0.14` to `0.08`;
  final nodes move from `163` to `166`, `Lpp` delta regresses from
...
- `.omo/evidence/task-139-chain32-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain32 now matches the oracle pair sequence
  `(2.8,11,3),(12.5,30,2),(16.25,34,4)`, removing the previous chain32
  residual prefix entries while leaving the chain32->30 and chain32->34
  contacts one-way; node-count mismatches improve from `8` to `7`, pair
  mismatches improve from `30` to `27`, and `Z` delta improves from `0.08` to
  `0.06`; final nodes move from `166` to `167`, `Lpp` delta regresses from
...
- `.omo/evidence/task-140-chain34-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain34 now matches the oracle pair sequence
  `(4.45,28,1),(7.87,48,4),(11.25,30,3),(14.63,28,1)`, removing the
  previous chain34 residual prefix entries while suppressing the oracle-absent
  chain2 reciprocal on chain34; node-count mismatches improve from `7` to `6`,
  pair mismatches improve from `27` to `20`, and `Z` delta improves from
  `0.06` to `0.04`; final nodes move from `167` to `168`, `Lpp` delta
  regresses from `0.543559` to `0.643048`, and summary mismatches remain `9`,
  so chain37/39/40/42/46/48/49 geometry, final node count, pair details, and
  summary parity remain open
- `.omo/evidence/task-141-chain37-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain37 now matches the oracle pair sequence
  `(4.15,11,2),(7.27,4,2),(10.38,6,2)`, removing the previous chain37 residual
  prefix entries while keeping the chain37 lower-index contacts one-way;
  node-count mismatches improve from `6` to `5`, pair mismatches improve from
  `20` to `18`, and `Z` delta improves from `0.04` to `0.02`; final nodes
  move from `168` to `169`, `Lpp` delta regresses from `0.643048` to
  `0.680981`, and summary mismatches remain `9`, so chain39/40/42/46/48/49
  geometry, final node count, pair details, and summary parity remain open
- `.omo/evidence/task-142-chain39-spplus-residual/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain39 now matches the oracle pair sequence
  `(3.71,43,1),(6.65,4,3),(9.82,18,2),(13.16,11,3)`, removing the previous
  chain39 residual prefix entries while keeping the lower-index chain39
  contacts one-way and preserving the oracle chain43 reciprocal source/node;
  pair mismatches improve from `18` to `11`, final nodes move from `169` to
  `167`, and `Lpp` delta improves from `0.680981` to `0.625463`; node-count
  mismatches remain `5`, `Z` delta regresses from `0.02` to `0.06`, and
  summary mismatches remain `9`, so chain40/42/43/46/48/49 geometry, final
  node count, pair details, and summary parity remain open
- `.omo/evidence/task-145-benchmark05-chain42-pair34/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain42 now matches the oracle one-way pair `(3.83,34,1)`.
  Pair mismatches improve from `11` to `8`, node-count mismatches improve from
  `5` to `4`, `Z` delta improves from `0.06` to `0.04`, and final nodes move
  from `167` to `168`; `Lpp` delta regresses from `0.625463` to `0.637117`,
  summary mismatches remain `9`, so chain40/43/46/48/49 geometry, reciprocal
  coverage, final node count, pair details, and summary parity remain open
- `.omo/evidence/task-146-benchmark05-chain43-pair28-prune/benchmark-04-05-spplus.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain43 now matches the oracle pair sequence `(4.17,39,1)` after
  pruning the oracle-absent pair28 preserved node at source bead `18.25`.
  Node-count mismatches improve from `4` to `3` and `Lpp` delta improves from
  `0.637117` to `0.630778`; pair mismatches move from `8` to `9`, `Z` delta
  moves from `0.04` to `0.06`, final nodes move from `168` to `167`, and
  summary mismatches remain `9`, so chain40/46/48/49 geometry, reciprocal
  coverage, final node count, pair details, and summary parity remain open
- `.omo/evidence/task-147-benchmark05-chain40-pairs/benchmark-04-05-spplus-final.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain40 now matches the oracle pair sequence
  `(3.59,25,3),(7.07,1,2),(14.96,4,1)`. Pair mismatches improve from `9` to
  `8`, while node-count mismatches remain `3`, `Lpp` delta remains `0.630778`,
  `Z` delta remains `0.06`, final nodes remain `167`, and summary mismatches
  remain `9`, so chain46/48/49 geometry, reciprocal coverage, final node
  count, pair details, and summary parity remain open
- `.omo/evidence/task-148-benchmark05-chain46-pairs/benchmark-04-05-spplus-final2.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain46 now matches the oracle pair sequence
  `(4.11,28,1),(7.33,31,2),(10.62,39,1)`. Pair mismatches improve from `8` to
  `4`, node-count mismatches improve from `3` to `2`, `Z` delta improves from
  `0.06` to `0.04`, final nodes move from `167` to `168`, and summary
  mismatches remain `9`, so chain48/49 geometry, reciprocal coverage, final
  node count, pair details, and summary parity remain open
- `.omo/evidence/task-149-benchmark05-chain48-pairs/benchmark-04-05-spplus-final.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain48 now matches the oracle pair sequence
  `(2.58,18,2),(5.69,49,2),(12.53,30,1),(16.26,34,3)`. Pair mismatches improve
  from `4` to `1`, node-count mismatches improve from `2` to `1`, `Z` delta
  improves from `0.04` to `0.02`, final nodes move from `168` to `169`, and
  summary mismatches remain `9`, so chain49 reciprocal source/position
  placement, final node count, pair details, and summary parity remain open
- `.omo/evidence/task-150-benchmark05-chain49-pair/benchmark-04-05-spplus-final.md`:
  benchmark-04 SP+ remains `passed`; benchmark-05 SP+ remains `mismatch`, but
  benchmark-05 chain49 now matches the oracle reciprocal pair `(14.67,48,2)`.
  Pair mismatches improve from `1` to `0`, node-count mismatches improve from
  `1` to `0`, final nodes move from `169` to `170`, source residual details
  clear to `none`, and `Z` delta improves from `0.02` to `0`; summary
  mismatches improve from `9` to `6`, so benchmark-05 final geometry, Lpp, and
  summary parity remain open
- `.omo/evidence/task-151-chain-contour-diagnostics/benchmark-04-05-spplus-final.md`:
  ...
  `none`, and `Z` delta `0`. Task-151 adds diagnostic-only chain-contour
  residual reporting and localizes the largest benchmark-05 remaining contour
  residual to chain39 (`max chain contour delta = 9.45416`) while preserving
  the open `Lpp` delta `0.754638` and six summary mismatches.
...
| --- | --- | --- |
| Full default/SP+ numerical parity | Benchmark-04 default/SP+ now report `passed` using formatted summary parity plus SP geometry/pairing checks and `source_mismatches=0`; benchmark-03 SP+ now matches first-chain obstacle sequence and reports zero node/pair mismatches, but task-66 records four source-bead residual details and a remaining max residual of `1.4679581658620817` plus summary/geometry mismatch; benchmark-01/02/05 remain `mismatch`; task-68 shows benchmark-01/02 oracle dumbbell obstacles are covered by a broader convex-hull candidate surface but with many extra candidates, task-69 shows the current source-gap/y-min convex selection still misses 11 benchmark-01 and 8 benchmark-02 oracle obstacles, task-70 records large oracle-obstacle source residuals even for covered oracle obstacles, task-71 rules out current blocked trace source carrying, task-72 shows oracle source assignment often uses non-nearest first-chain segments, task-73 shows source/order is a default-reducer output rather than SP+ pair annotation, task-74 confirms the Z1+ oracle core/final scan counters are now visible from `run.stdout`, task-75 turns source/order divergence into direct pyz1-vs-default-oracle sequence mismatch counts of 13/10/4/2 for benchmarks 01/02/03/05, task-76 records those divergences per source index including oracle-only trailing sources, task-86 makes benchmark-05's true-chain interaction gap direct with pyz1 pair `4` versus oracle `40,26`, task-87 shows oracle `40,26` are present in a true-chain contact candidate surface `6,40,26,12`, task-88 shows oracle default source nearest-contact selection recovers `40,26`, task-89 makes pyz1 retain true-chain pair sequence `40,26` for benchmark-05, task-90 exposes the benchmark-05 true-chain pair node-index gap as pyz1 `11,1` versus oracle `3,2`, task-91 aligns that node-index sequence to `3,2` while reducing pair mismatches from 70 to 68, tasks 92-150 progressively improve benchmark-05 reciprocal and local true-chain pair coverage through chain49, chain20, chain30, chain31, chain32, chain34, chain37, chain39, chain40, chain42, chain43, chain46, and chain48; task-150 aligns chain49 to oracle reciprocal pair `(14.67,48,2)` and closes benchmark-05 SP+ pair mismatches, node-count mismatches, source residuals, and `Z` delta; task-151 adds chain-contour residual diagnostics and localizes the largest remaining benchmark-05 contour gap to chain39; task-152 closes that chain39 contour residual and moves the largest remaining contour gap to chain48; task-153 closes the chain48 contour residual and moves the largest remaining contour gap to chain34; task-154 closes the chain34 contour residual and moves the largest remaining contour gap to chain36; task-155 closes the chain36 contour residual and moves the largest remaining contour gap to chain27; task-156 closes the chain27 contour residual and moves the largest remaining contour gap to chain25; task-157 closes the chain25 contour residual and moves the largest remaining contour gap to chain9; task-158 closes the chain9 contour residual and moves the largest remaining contour gap to chain43; task-159 closes the chain43 contour residual and moves the largest remaining contour gap to chain22; task-160 closes the chain22 contour residual and moves the largest remaining contour gap to chain37 while final geometry, `Lpp`, and summary mismatches remain open; the public Z1+ tree lacks `module-Z1.f90`; benchmark-06+ remain guarded | Reported `passed` status or documented scientifically acceptable tolerance for all intended default/SP+ cases |
...
3. If scientific parity is the priority, continue reducer geometry debugging
   ...
   task-99 adds second pair-13 coverage, task-141 has progressed local
   pair-sequence coverage through benchmark-05 chain37, task-142 aligns
   benchmark-05 chain39, task-145 aligns benchmark-05 chain42, task-146
   aligns benchmark-05 chain43 by pruning the oracle-absent pair28 node, and
   task-147 aligns benchmark-05 chain40, task-148 aligns benchmark-05
   chain46, task-149 aligns benchmark-05 chain48, task-150 aligns
   benchmark-05 chain49, task-151 localizes the largest contour gap to
   chain39, task-152 closes chain39 contour placement, and task-153 closes
   chain48 contour placement; task-154 closes chain34 contour placement, and
   task-155 closes chain36 contour placement, task-156 closes chain27
   contour placement, task-157 closes chain25 contour placement, task-158
   closes chain9 contour placement, task-159 closes chain43 contour
   placement, and task-160 closes chain22 contour placement, so the next
   reducer gap starts at chain37 final
   geometry/contour, Lpp, and summary alignment. Use task-62/task-68 winding
