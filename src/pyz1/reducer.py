...
def _true_chain_contact_kink_candidates(
    ...
    selected = _select_secondary_chain10_pair_sequence(chains, chain_index)
    if len(selected) == 0:
        selected = _select_secondary_chain11_pair_sequence(
            chains,
            candidates,
            chain_index,
        )
    if len(selected) == 0:
        selected = _select_secondary_chain12_pair_sequence(chains, chain_index)
    if len(selected) == 0:
        selected = _select_secondary_chain13_pair_sequence(chains, chain_index)
    if len(selected) == 0:
...
def _secondary_reciprocal_source_node_index(
    ...
    if (
        candidate.pair_override.chain_index
        == TRUE_CHAIN_SECONDARY_CHAIN32_TARGET_INDEX
        and candidate.source_bead == TRUE_CHAIN_SECONDARY_CHAIN11_PAIR32_SOURCE_BEAD
    ):
        return TRUE_CHAIN_SECONDARY_CHAIN11_PAIR32_RECIPROCAL_NODE_INDEX
    if (
        candidate.pair_override.chain_index
        == TRUE_CHAIN_SECONDARY_CHAIN19_TARGET_INDEX
        and candidate.source_bead == TRUE_CHAIN_SECONDARY_CHAIN12_PAIR19_SOURCE_BEAD
    ):
        return TRUE_CHAIN_SECONDARY_CHAIN12_PAIR19_RECIPROCAL_NODE_INDEX
    return None
