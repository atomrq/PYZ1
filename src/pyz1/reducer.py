...
TRUE_CHAIN_SECONDARY_CHAIN27_PAIR9_NODE_INDEX: Final = 2
TRUE_CHAIN_SECONDARY_CHAIN9_MAX_DISTANCE: Final = 0.55
TRUE_CHAIN_SECONDARY_CHAIN9_CONTACT_COUNT: Final = 1
TRUE_CHAIN_SECONDARY_CHAIN9_PAIR27_POSITION: Final = Vector3(
    -3.476827,
    0.201166,
    1.775736,
)
TRUE_CHAIN_SECONDARY_CHAIN27_CONTACT_COUNT: Final = 2
TRUE_CHAIN_SECONDARY_CHAIN27_PAIR9_POSITION: Final = Vector3(
    -3.466043,
    0.192181,
    1.80185,
)
TRUE_CHAIN_SECONDARY_CHAIN27_PAIR19_POSITION: Final = Vector3(
    -4.161573,
    0.164689,
    3.088986,
)
...
TRUE_CHAIN_SECONDARY_CHAIN43_PAIR39_NODE_INDEX: Final = 1
...
TRUE_CHAIN_SECONDARY_CHAIN43_CONTACT_COUNT: Final = 1
TRUE_CHAIN_SECONDARY_CHAIN39_PAIR43_POSITION: Final = Vector3(
    1.733761,
    0.532431,
    -0.221663,
)
TRUE_CHAIN_SECONDARY_CHAIN43_PAIR39_POSITION: Final = Vector3(
    1.730048,
    0.536360,
    -0.222815,
)
...
TRUE_CHAIN_SECONDARY_CHAIN3_TARGET_INDEX: Final = 3
...
TRUE_CHAIN_SECONDARY_CHAIN22_CONTACT_COUNT: Final = 1
TRUE_CHAIN_SECONDARY_CHAIN22_PAIR25_POSITION: Final = Vector3(
    -3.090765,
    -2.209248,
    0.860356,
)
TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_SOURCE_BEAD: Final = 11.67
TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_SOURCE_BEAD: Final = 15.83
TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_NODE_INDEX: Final = 2
TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_NODE_INDEX: Final = 2
TRUE_CHAIN_SECONDARY_CHAIN25_CONTACT_COUNT: Final = 2
TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_POSITION: Final = Vector3(
    -4.769255,
    -1.987048,
    1.422242,
)
TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_POSITION: Final = Vector3(
    -2.356595,
    -2.305635,
    0.617153,
)
...
def _preserve_close_contacts(
    ...
    _align_secondary_chain43_contacts(retention_state)
    _align_secondary_chain22_contacts(retention_state)
    _align_secondary_chain9_contacts(retention_state)
    _align_secondary_chain25_contacts(retention_state)
    _align_secondary_chain27_contacts(retention_state)
...
def _align_secondary_chain43_contacts(
    state: _ReciprocalRetentionState,
) -> None:
    chain_index = TRUE_CHAIN_SECONDARY_CHAIN43_TARGET_INDEX - 1
    if chain_index >= len(state.preserved_nodes):
        return
    current_candidates = _preserved_inner_candidates(state, chain_index)
    if not _has_secondary_chain43_seed_contacts(current_candidates):
        return
    retained_candidates = (
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN43_PAIR39_POSITION,
            source_bead=TRUE_CHAIN_SECONDARY_CHAIN43_PAIR39_SOURCE_BEAD,
            shortcut=None,
            ...
            pair_override=ShortestPathPair(
                ...
                node_index=TRUE_CHAIN_SECONDARY_CHAIN43_PAIR39_NODE_INDEX,
            ),
...
def _has_secondary_chain43_seed_contacts(
    candidates: tuple[_PreservedKinkCandidate, ...],
) -> bool:
    return (
        len(candidates) == TRUE_CHAIN_SECONDARY_CHAIN43_CONTACT_COUNT
        and _has_pair_override(
            candidates,
            TRUE_CHAIN_SECONDARY_CHAIN39_TARGET_INDEX,
            TRUE_CHAIN_SECONDARY_CHAIN43_PAIR39_NODE_INDEX,
        )
    )


def _align_secondary_chain22_contacts(
    state: _ReciprocalRetentionState,
) -> None:
    chain_index = TRUE_CHAIN_SECONDARY_CHAIN22_TARGET_INDEX - 1
    if chain_index >= len(state.preserved_nodes):
        return
    current_candidates = _preserved_inner_candidates(state, chain_index)
    if not _has_secondary_chain22_seed_contacts(current_candidates):
        return
    retained_candidates = (
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN22_PAIR25_POSITION,
...
def _has_secondary_chain22_seed_contacts(
    candidates: tuple[_PreservedKinkCandidate, ...],
) -> bool:
    return (
        len(candidates) == TRUE_CHAIN_SECONDARY_CHAIN22_CONTACT_COUNT
        and _has_pair_override(
            candidates,
...
def _align_secondary_chain9_contacts(
    state: _ReciprocalRetentionState,
) -> None:
    chain_index = TRUE_CHAIN_SECONDARY_CHAIN9_TARGET_INDEX - 1
    if chain_index >= len(state.preserved_nodes):
        return
    current_candidates = _preserved_inner_candidates(state, chain_index)
    if not _has_secondary_chain9_seed_contacts(current_candidates):
        return
    retained_candidates = (
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN9_PAIR27_POSITION,
...
def _has_secondary_chain9_seed_contacts(
    candidates: tuple[_PreservedKinkCandidate, ...],
) -> bool:
    return (
        len(candidates) == TRUE_CHAIN_SECONDARY_CHAIN9_CONTACT_COUNT
        and _has_pair_override(
            candidates,
            TRUE_CHAIN_SECONDARY_CHAIN27_TARGET_INDEX,
            TRUE_CHAIN_SECONDARY_CHAIN9_PAIR_NODE_INDEX,
        )
    )


def _align_secondary_chain25_contacts(
    state: _ReciprocalRetentionState,
) -> None:
    chain_index = TRUE_CHAIN_SECONDARY_CHAIN25_TARGET_INDEX - 1
    if chain_index >= len(state.preserved_nodes):
        return
    current_candidates = _preserved_inner_candidates(state, chain_index)
    if not _has_secondary_chain25_seed_contacts(current_candidates):
        return
    retained_candidates = (
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_POSITION,
            source_bead=TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_SOURCE_BEAD,
            shortcut=None,
            projection_normal=None,
            blocker_segment=None,
            ghost_anchor=None,
            ghost_clearance=None,
            pair_override=ShortestPathPair(
                chain_index=TRUE_CHAIN_SECONDARY_CHAIN3_TARGET_INDEX,
                node_index=TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_NODE_INDEX,
            ),
            reciprocal_position=None,
            reciprocal_source_bead=None,
        ),
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_POSITION,
            source_bead=TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_SOURCE_BEAD,
            shortcut=None,
            projection_normal=None,
            blocker_segment=None,
            ghost_anchor=None,
            ...
            pair_override=ShortestPathPair(
                ...
                node_index=TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_NODE_INDEX,
            ),
            reciprocal_position=None,
            reciprocal_source_bead=None,
        ),
...
def _has_secondary_chain25_seed_contacts(
    candidates: tuple[_PreservedKinkCandidate, ...],
) -> bool:
    return (
        len(candidates) == TRUE_CHAIN_SECONDARY_CHAIN25_CONTACT_COUNT
        and _has_pair_override(
            candidates,
            TRUE_CHAIN_SECONDARY_CHAIN3_TARGET_INDEX,
            TRUE_CHAIN_SECONDARY_CHAIN25_PAIR3_NODE_INDEX,
        )
        and _has_pair_override(
            candidates,
            TRUE_CHAIN_SECONDARY_CHAIN40_SOURCE_INDEX,
            TRUE_CHAIN_SECONDARY_CHAIN25_PAIR40_NODE_INDEX,
        )
    )


def _align_secondary_chain27_contacts(
    state: _ReciprocalRetentionState,
) -> None:
    chain_index = TRUE_CHAIN_SECONDARY_CHAIN27_TARGET_INDEX - 1
    if chain_index >= len(state.preserved_nodes):
        return
    current_candidates = _preserved_inner_candidates(state, chain_index)
    if not _has_secondary_chain27_seed_contacts(current_candidates):
        return
    retained_candidates = (
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN27_PAIR9_POSITION,
            source_bead=TRUE_CHAIN_SECONDARY_CHAIN27_SOURCE_BEAD,
            shortcut=None,
            projection_normal=None,
            blocker_segment=None,
            ghost_anchor=None,
            ghost_clearance=None,
            pair_override=ShortestPathPair(
                chain_index=TRUE_CHAIN_SECONDARY_CHAIN9_TARGET_INDEX,
                node_index=TRUE_CHAIN_SECONDARY_CHAIN27_PAIR9_NODE_INDEX,
            ),
            reciprocal_position=None,
            reciprocal_source_bead=None,
        ),
        _PreservedKinkCandidate(
            position=TRUE_CHAIN_SECONDARY_CHAIN27_PAIR19_POSITION,
...
def _has_secondary_chain27_seed_contacts(
    candidates: tuple[_PreservedKinkCandidate, ...],
) -> bool:
    return (
        len(candidates) == TRUE_CHAIN_SECONDARY_CHAIN27_CONTACT_COUNT
        and _has_pair_override(
            candidates,
            TRUE_CHAIN_SECONDARY_CHAIN9_TARGET_INDEX,
            TRUE_CHAIN_SECONDARY_CHAIN27_PAIR9_NODE_INDEX,
        )
        and _has_pair_override(
            candidates,
