...
def test_reduce_snapshot_when_benchmark05_chain25_contour_matches_oracle() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[24])
    expected_contour = _shortest_path_chain_contour(oracle.chains[24])
...
def test_reduce_snapshot_when_benchmark05_chain22_contour_matches_oracle() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
    oracle = read_shortest_path_file(
        ORACLE_ROOT / "benchmark-05" / "spplus" / "Z1+SP.dat",
    )
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[21])
    expected_contour = _shortest_path_chain_contour(oracle.chains[21])
    assert abs(actual_contour - expected_contour) < 0.001
...
def test_reduce_snapshot_when_benchmark05_chain27_contour_matches_oracle() -> None:
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[26])
    expected_contour = _shortest_path_chain_contour(oracle.chains[26])
    assert abs(actual_contour - expected_contour) < 0.001
...
def test_reduce_snapshot_when_benchmark05_chain36_contour_matches_oracle() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
    oracle = read_shortest_path_file(
        ORACLE_ROOT / "benchmark-05" / "spplus" / "Z1+SP.dat",
    )
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[35])
    expected_contour = _shortest_path_chain_contour(oracle.chains[35])
    assert abs(actual_contour - expected_contour) < 0.001
...
def test_reduce_snapshot_when_benchmark05_chain34_contour_matches_oracle() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
    oracle = read_shortest_path_file(
        ORACLE_ROOT / "benchmark-05" / "spplus" / "Z1+SP.dat",
    )
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[33])
    expected_contour = _shortest_path_chain_contour(oracle.chains[33])
    assert abs(actual_contour - expected_contour) < 0.001
...
def test_write_benchmark_report_when_contact_relaxation_measures_guarded_spplus(
    ...
    assert all(delta is not None for delta in summary_contract_deltas)
    ...
    assert records[0].ne_classical_kink_delta < 0.001
    assert records[0].ne_modified_kink_delta is not None
    assert records[0].ne_modified_kink_delta < 0.001
    ...
    assert "b0 delta | density delta" in text
...
def test_reduce_snapshot_when_benchmark05_chain46_matches_oracle_pairs() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")

    result = reduce_snapshot(snapshot, ReducerSettings(pairing_enabled=True))

    chain_46_pairs = tuple(
        (
            node.source_bead,
            node.pair.chain_index,
            node.pair.node_index,
        )
        for node in result.shortest_path.chains[45].nodes[1:-1]
        if node.pair is not None
    )
    assert chain_46_pairs == (
        (4.11, 28, 1),
        (7.33, 31, 2),
        (10.62, 39, 1),
    )
...
def test_reduce_snapshot_when_benchmark05_chain48_matches_oracle_pairs() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
...
def test_reduce_snapshot_when_benchmark05_chain48_contour_matches_oracle() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
    oracle = read_shortest_path_file(
        ORACLE_ROOT / "benchmark-05" / "spplus" / "Z1+SP.dat",
    )
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[47])
    expected_contour = _shortest_path_chain_contour(oracle.chains[47])
    assert abs(actual_contour - expected_contour) < 0.001
...
def test_reduce_snapshot_when_benchmark05_chain42_matches_oracle_pair() -> None:
    ...
    chain_42_pairs = tuple(
        (
            node.source_bead,
            node.pair.chain_index,
            node.pair.node_index,
        )
        for node in result.shortest_path.chains[41].nodes[1:-1]
        if node.pair is not None
    )
    assert chain_42_pairs == ((3.83, 34, 1),)


def test_reduce_snapshot_when_benchmark05_chain43_matches_oracle_pair() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")

    result = reduce_snapshot(snapshot, ReducerSettings(pairing_enabled=True))

    chain_43_pairs = tuple(
        (
            node.source_bead,
            node.pair.chain_index,
            node.pair.node_index,
        )
        for node in result.shortest_path.chains[42].nodes[1:-1]
        if node.pair is not None
    )
    assert chain_43_pairs == ((4.17, 39, 1),)


def test_reduce_snapshot_when_benchmark05_chain43_contour_matches_oracle() -> None:
    snapshot = read_z1_file(SOURCE_Z1 / ".benchmark-05.Z1")
    oracle = read_shortest_path_file(
        ORACLE_ROOT / "benchmark-05" / "spplus" / "Z1+SP.dat",
    )
    ...
    actual_contour = _shortest_path_chain_contour(result.shortest_path.chains[42])
    expected_contour = _shortest_path_chain_contour(oracle.chains[42])
    assert abs(actual_contour - expected_contour) < 0.001
